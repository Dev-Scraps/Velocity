...
import type { AudioTrack } from "../../hooks/useRustCommands";
...
export const AudioPanel = ({ audioTracks, selectedAudioUrl, onSelectAudio, onBack }: AudioPanelProps) => (
  <div className="absolute z-50 rounded-3xl overflow-hidden border border-white/20 bg-black/40 backdrop-blur-xl shadow-2xl animate-fadeIn hover:shadow-white/10 transition-all duration-300 bottom-20 sm:bottom-24 lg:bottom-16 right-4 sm:right-6 w-80">
    <div className="flex items-center justify-between p-4">
      ...
      <span className="text-white font-semibold text-base tracking-wide">Audio Tracks</span>
      <div className="w-6" />
    </div>
    <div className="p-3 space-y-2 max-h-64 overflow-y-auto">
      {audioTracks.map((track) => (
        <button
          ...
          className={`w-full text-left px-3 py-2 rounded-lg border border-white/10 transition-all duration-300 hover:scale-105 active:scale-95 backdrop-blur-sm ${selectedAudioUrl === track.url ? "bg-white/20 text-white shadow-lg shadow-white/20" : "bg-white/5 text-white/80 hover:bg-white/10"}`}
        >
          {track.name || track.language || track.format_id}
        </button>
      ))}
      {audioTracks.length === 0 && (
        <div className="text-white/60 text-sm">No alternate audio tracks.</div>
      )}
    </div>
  </div>
);
